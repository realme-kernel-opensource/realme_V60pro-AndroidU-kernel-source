...
#include "hi846mipiraw_rado_Sensor.h"
...
static BYTE hi846_rado_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };
static DEFINE_SPINLOCK(imgsensor_drv_lock);
//static kal_uint8 otp_data[0x2000] = {0};
#define per_frame 1

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846_SENSOR_ID_RADO,
	...
	.pre = {
		...
		.max_framerate = 300,
		.mipi_pixel_rate = 271200000, //(678M*4/10)
	},
	.cap = {
		.pclk = 288000000,
		.linelength = 3800,
	...
	.hs_video = {
		.pclk = 288000000,
		.linelength = 3800, 			//record different mode's linelength
		.framelength = 2525, 		//record different mode's framelength
		.startx = 0,
		.starty = 0,
		...
		.max_framerate = 300,
		.mipi_pixel_rate = 135600000, //(339M*4/10)
	},
	.slim_video = {
		.pclk = 288000000,
		.linelength = 3800,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[9] =
{
 ...
 { 3280, 2464,   8, 314,   3264, 1836,   3264, 1836,   0, 0, 3264, 1836,   0, 0, 3264, 1836}, // video
 { 3280, 2464,   8,   8,   3264, 2448,   1632, 1224,   0, 0, 1632, 1224,   0, 0, 1632, 1224}, // custom video
 ...
 { 3280, 2464,   8,   8,   3264, 2448,   1632, 1224,   0, 0, 1632, 1224,   0, 0, 1632, 1224}, // custom3
 { 3280, 2464, 480, 360,   2320, 1744,   2320, 1744,   0, 0, 2320, 1744,   0, 0, 2320, 1744}, // custom4
};// slim video
...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
    ...
    printk("yujinghong - i2c_write_id = 0x%x\n", imgsensor.i2c_write_id);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x090C, 0x03F1);
	write_cmos_sensor(0x090E, 0x000B);
	write_cmos_sensor(0x0954, 0x0089);
	write_cmos_sensor(0x0956, 0x0000);
	write_cmos_sensor(0x0958, 0xA980);
	...
	write_cmos_sensor(0x0F38, 0x0371);
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x091C, 0x0E08);
	write_cmos_sensor(0x091E, 0x0A00);
	write_cmos_sensor(0x090C, 0x03F5);
...
static void custom4_setting(void)
{
	...
	//Date		  : 2020-11-13
	//Customer        : CH
	//Image size      : 2320x1744
	//MCLK/PCLK       : 24MHz /288Mhz
	//MIPI speed(Mbps): 720Mbps x 4Lane
	//Frame Length    : 2526
	//Line Length     : 3800
	//Max Fps         : 30.00fps(vblank>10.3ms)
	//Pixel order     : Green 1st (=GB)
	//X/Y-flip        : X-flip
	//Firmware Ver.   : v10.5.1
	////////////////////////////////////////////////
	write_cmos_sensor(0x002E, 0x1111);
	write_cmos_sensor(0x0032, 0x1111);
	write_cmos_sensor(0x0026, 0x01A0);
	write_cmos_sensor(0x002C, 0x086F);
	write_cmos_sensor(0x005C, 0x2101);
	write_cmos_sensor(0x0006, 0x09DE);//
	write_cmos_sensor(0x0008, 0x0ED8);
	write_cmos_sensor(0x000C, 0x0022);
	write_cmos_sensor(0x0A22, 0x0000);
	write_cmos_sensor(0x0A24, 0x0000);
	write_cmos_sensor(0x0804, 0x01D8);
	write_cmos_sensor(0x0A12, 0x0910);
	write_cmos_sensor(0x0A14, 0x06D0);
	write_cmos_sensor(0x0074, 0x09D8);//
	write_cmos_sensor(0x021C, 0x0001);
	write_cmos_sensor(0x0A04, 0x014A);
	write_cmos_sensor(0x0418, 0x026C);
	write_cmos_sensor(0x0128, 0x0028);
	write_cmos_sensor(0x012A, 0xFFFF);
	write_cmos_sensor(0x0120, 0x0046);
	write_cmos_sensor(0x0122, 0x0376);
	...
	write_cmos_sensor(0x2008, 0x38FD);
	write_cmos_sensor(0x326E, 0x0000);
	write_cmos_sensor(0x0710, 0x06F0);
	write_cmos_sensor(0x0900, 0x0300);
	write_cmos_sensor(0x0902, 0xC319);
	write_cmos_sensor(0x0914, 0xC109);
	write_cmos_sensor(0x0916, 0x061A);
...
static kal_uint16 read_hi846_rado_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
...
static void read_hi846_rado_module_data()
{
...
// sensorid : [30 ~ 33]
	...
	memset(hi846_rado_common_data, 0, sizeof(hi846_rado_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		hi846_rado_common_data[8 + idx] = read_hi846_rado_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	...
	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		printk("cam data, idx: %02x %02x %02x %02x %d\n",
		       hi846_rado_common_data[idx],
		       hi846_rado_common_data[idx + 1],
		       hi846_rado_common_data[idx + 2],
		       hi846_rado_common_data[idx + 3],
			   idx);
}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
            printk("i2c add= 0x%x modules_sensorid=0x%x\n",imgsensor.i2c_write_id,*sensor_id);
            printk("driver_sensorid=0x%x\n",imgsensor_info.sensor_id);

			if (*sensor_id == imgsensor_info.sensor_id - 0x1) {
...
static kal_uint32 open(void)
{
	...
	if (imgsensor_info.sensor_id - 0x1 != sensor_id) {
...
//-Bug642680 , sunyunpeng.wt, 20210414, add ,fot its tecxt_sensor_fusion.
	...
	case SENSOR_FEATURE_GET_EEPROM_COMDATA:
		memcpy(feature_return_para_32, hi846_rado_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);
...
UINT32 HI846_MIPI_RAW_RADO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc != NULL)
		*pfFunc =  &sensor_func;
	return ERROR_NONE;
}	/*	HI846_MIPI_RAW_RADO_SensorInit	*/

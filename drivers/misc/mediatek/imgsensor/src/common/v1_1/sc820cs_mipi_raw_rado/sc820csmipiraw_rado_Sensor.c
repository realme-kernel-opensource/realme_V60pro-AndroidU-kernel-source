...
#include "sc820csmipiraw_rado_Sensor.h"
#define PFX "sc820cs_truly_main_i_camera_sensor"
#define LOG_INF(format, args...)    \
    pr_err(PFX "[%s] " format, __func__, ##args)
//static unsigned char fusion_id_main[96];
//static unsigned char sn_main[96];
static DEFINE_SPINLOCK(imgsensor_drv_lock);
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = SC820CS_SENSOR_ID_RADO,
...
#define SC820CS_RADO_EEPROM_I2C_ADDR 0xA8
static BYTE sc820cs_rado_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

static kal_uint16 read_sc820cs_rado_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, SC820CS_RADO_EEPROM_I2C_ADDR);

	return get_byte;
}
static void read_sc820cs_rado_module_data()
{
...
// sensorid : [30 ~ 33]
    ...
    memset(sc820cs_rado_common_data, 0,sizeof(sc820cs_rado_common_data));
    // QR
    for(idx = 0; idx < sn_length; idx++)
    {
        sc820cs_rado_common_data[8 + idx] = read_sc820cs_rado_eeprom_module(sn_starAddr + idx);
    }
    //vcm
    sc820cs_rado_common_data[40] = read_sc820cs_rado_eeprom_module(vcmAddr);
    sc820cs_rado_common_data[41] = read_sc820cs_rado_eeprom_module(vcmAddr + 1);
    //lensid
    sc820cs_rado_common_data[44] = read_sc820cs_rado_eeprom_module(lensAddr);
    sc820cs_rado_common_data[45] = read_sc820cs_rado_eeprom_module(lensAddr + 1);

    for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4) {
        LOG_INF("cam data: %02x %02x %02x %02x\n",
               sc820cs_rado_common_data[idx],
               sc820cs_rado_common_data[idx + 1],
               sc820cs_rado_common_data[idx + 2],
               sc820cs_rado_common_data[idx + 3]);
    }
}
...
static kal_uint32 open(void)
{
    ...
    LOG_INF("[open]: PLATFORM:MT6737,MIPI 2LANE\n");
    LOG_INF("preview 3264*2448@30fps,1320Mbps/lane;"
        "capture 3264*2448@30fps,1320Mbps/lane\n");
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        spin_lock(&imgsensor_drv_lock);
        imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
        spin_unlock(&imgsensor_drv_lock);
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
    ...
    if (enable) {
		...
		write_cmos_sensor8(0x5000,0x08);
            ...
            UINT8 *feature_para, UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        memcpy(feature_return_para_32, sc820cs_rado_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);

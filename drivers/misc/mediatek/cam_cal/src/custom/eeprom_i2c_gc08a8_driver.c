...
#define OTP_LENGTH 1971
#include "eeprom_i2c_gc08a8_driver.h"
	...
	unsigned int ui4_length, unsigned char *pinputdata)
{
	...
	printk("gc08a8 otp ui4_offset = 0x%x, ui4_length = %d \n", ui4_offset, ui4_length);

	gc08a8_otp_init();
	mdelay(10);

	i4RetValue = gc08a8_otp_read_group(0x15a0, pinputdata,OTP_LENGTH);
	//i4RetValue = gc08a8_otp_read_group(0x15a0, pinputdata, 0x7B1);
	pr_debug("gc08a8 otp i4RetValue:%d \n", i4RetValue);
	if (i4RetValue != 0) {

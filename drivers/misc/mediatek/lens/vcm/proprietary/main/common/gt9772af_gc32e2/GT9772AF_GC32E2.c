...
/*
 * GT9772AF_GC32E2 voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "GT9772AF_GC32E2_DRV"
...
static int initAF(void)
{
    ...
    if (*g_pAF_Opened == 1) {
        ...
        ret = s4AF_WriteReg(0, 0x06, 0x84); //AAC4
        LOG_INF("reg 06 ret: %x\n", ret);
        ret = s4AF_WriteReg(0, 0x07, 0x01);
        LOG_INF("reg 07 ret: %x\n", ret);
        ret = s4AF_WriteReg(0, 0x08, 0x56); //12.8ms
        LOG_INF("reg 08 ret: %x\n", ret);

        s4AF_ReadReg(0x06, &Temp);  //READ 0x06
        LOG_INF("Reg 06 now is : %x\n", Temp);
        s4AF_ReadReg(0x07, &Temp);  //read 0x07
        LOG_INF("Reg 07 now is : %x\n", Temp);
        s4AF_ReadReg(0x08, &Temp);  //read 0x08
        LOG_INF("Reg 08 now is : %x\n", Temp);
...
/* ////////////////////////////////////////////////////////////// */
long GT9772AF_GC32E2_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int GT9772AF_GC32E2_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
    int Ret = 0;
...
int GT9772AF_GC32E2_PowerDown(struct i2c_client *pstAF_I2Cclient,
        int *pAF_Opened)
{
...
int GT9772AF_GC32E2_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int GT9772AF_GC32E2_GetFileName(unsigned char *pFileName)
{

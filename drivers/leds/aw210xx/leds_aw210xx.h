...
static AW210XX_CFG aw210xx_cfg_array[] = {
    ...
    {charge_effect_data1, sizeof(charge_effect_data1)/sizeof(AW_MULTI_BREATH_DATA_STRUCT)},
...
typedef struct effect_control {
	int cur_effect;
	int prev_effect;
	int state[AW210XX_LED_MAXMODE]; // 0 disable, 1 init, 2 running, 3 pause, 4 second stage running (for charger),5 second stage pause

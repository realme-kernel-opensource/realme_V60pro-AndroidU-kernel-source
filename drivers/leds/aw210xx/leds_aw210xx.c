...
void aw210xx_rgb_multi_breath_init(const AW_MULTI_BREATH_DATA_STRUCT *data, effect_select_t effect)
{
	...
	for (i = 0; i < RGB_NUM; i++) { //RGB_NUM is 4，表示4颗RGB灯
		...
		aw210xx_algo_data[effect][i].total_frames = (data[i].time[0] + 19) / 20 + 1;
		...
		if (aw210xx_algo_data[effect][i].cur_frame >= aw210xx_algo_data[effect][i].total_frames) {
			...
			if (update_frame_idx) {
				aw210xx_algo_data[effect][i].total_frames =
					(data[i].time[breath_cur_phase[effect][i]])/20 + 1;
...
void print_effectdata(effect_select_t effect)
{
	...
	for(i=0;i<aw210xx_cfg_array[effect].count;i++)
	{
		...
		len += snprintf(buf + len, BUF_SIZE - len,"%d,%d,", effect, i);
		for(j=0;j<6;j++)
		{
...
/*
void aw210xx_rgb_multi_breath(struct aw210xx *aw210xx, effect_select_t effect)
{
	uint8_t ret = 0;
	...
	effect_stop_val[effect] = 0;
	aw210xx_rgb_multi_breath_init(effect_data);
	aw210xx_frame_display();
	aw210xx_update_effect(aw210xx);
	while (1) {
		ret = check_effect_state(effect);
		if(ret != 0){
			AW_ERR(" exit1 \n");
			break;
		}
		...
		aw210xx_update_frame_idx(effect_data);
...
void display_charger_frame(int blevel)
{
	int brightness = 0xff & charge_color[blevel][8];
	struct aw210xx *aw210xx_id1 = aw210xx_g_chip[1][0];
	struct aw210xx *led = aw210xx_g_chip[0][0];
	int i =0;
	for (i=0; i<4; i++)
	{
		AW_ERR(" charge_color = %d,%d,%d,%d,%d,%d,%d,%d,%d",charge_color[i][0],charge_color[i][1],charge_color[i][2],charge_color[i][3],
		charge_color[i][4],charge_color[i][5],charge_color[i][6],charge_color[i][7],charge_color[i][8]);
	}
	AW_ERR(" frame entry \n");

	aw210xx_i2c_write(led, AW210XX_REG_BR00L, 0x00);
	aw210xx_i2c_write(led, AW210XX_REG_BR03L, 0x00);
	aw210xx_i2c_write(led, AW210XX_REG_BR06L, 0x00);
	aw210xx_i2c_write(led, AW210XX_REG_BR09L, 0x00);
	aw210xx_i2c_write(led, AW210XX_REG_BR01L, brightness);
	aw210xx_i2c_write(led, AW210XX_REG_BR04L, brightness);
	aw210xx_i2c_write(led, AW210XX_REG_BR07L, brightness);
	aw210xx_i2c_write(led, AW210XX_REG_BR10L, brightness);
	aw210xx_i2c_write(led, AW210XX_REG_BR02L, 0x00);
	aw210xx_i2c_write(led, AW210XX_REG_BR05L, 0x00);
	aw210xx_i2c_write(led, AW210XX_REG_BR08L, 0x00);
	aw210xx_i2c_write(led, AW210XX_REG_BR11L, 0x00);

	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR00L, 0x00);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR03L, 0x00);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR06L, 0x00);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR09L, 0x00);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR01L, brightness);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR04L, brightness);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR07L, brightness);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR10L, brightness);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR02L, 0x00);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR05L, 0x00);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR08L, 0x00);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_BR11L, 0x00);
	
	aw210xx_i2c_write(led, AW210XX_REG_SL00 + 1, charge_color[blevel][0]);
	aw210xx_i2c_write(led, AW210XX_REG_SL03 + 1, charge_color[blevel][1]);
	aw210xx_i2c_write(led, AW210XX_REG_SL06 + 1, charge_color[blevel][2]);
	aw210xx_i2c_write(led, AW210XX_REG_SL09 + 1, charge_color[blevel][3]);

	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_SL00 + 1, charge_color[blevel][4]);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_SL03 + 1, charge_color[blevel][5]);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_SL06 + 1, charge_color[blevel][6]);
	aw210xx_i2c_write(aw210xx_id1, AW210XX_REG_SL09 + 1, charge_color[blevel][7]);
...
void charger_effect_continue(struct aw210xx *aw210xx){
	//int enable = 0;
//	AW_ERR(" continue entry \n");
	if((blevel != effect_state.data[AW210XX_LED_CHARGE_MODE]%4) || last_run_effect != AW210XX_LED_CHARGE_MODE){
		blevel = effect_state.data[AW210XX_LED_CHARGE_MODE]%4;
		brightness_sbmd_setup(BR_RESOLUTION_8BIT);
		display_charger_frame(blevel);
	}
}
...
void run_charger_effect(struct aw210xx *aw210xx){
	...
	if(effect_state.state[AW210XX_LED_CHARGE_MODE] == 2){
		...
		if(ret == 1) // current effect cycle is completed
		{
			if(effect_state.state[AW210XX_LED_CHARGE_MODE] != 0) // if effect is not stopped then restart
			{
	...
	if(effect_state.state[AW210XX_LED_CHARGE_MODE] == 4){
		charger_effect_continue(aw210xx);
	}
}
...
void run_effect(struct aw210xx *aw210xx)
{
	while (1)
	{
		...
		case AW210XX_LED_CHARGE_MODE:
			AW_ERR(" AW210XX_LED_CHARGE_MODE enter \n" );
			aw210xx_rgb_multi_breath(aw210xx, CHARGE_EFFECT);
			charger_effect_continue(aw210xx);
			AW_ERR(" AW210XX_LED_CHARGE_MODE exit \n" );
			break;
		*/
	}
...
static ssize_t aw210xx_l_type_attr_show (struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "l_type %d = %d, %d =%d, %d =%d, %d =%d, %d =%d (max:%d)\n",L_BATTERY, l_type[L_BATTERY],L_NOTIFY, l_type[L_NOTIFY],L_KEYBOARD, l_type[L_KEYBOARD],L_GAMES, l_type[L_GAMES],L_INCALL, l_type[L_INCALL],L_WIFI, l_type[L_WIFI],L_MAX);
}
	...
	struct device_attribute *attr, const char *buf, size_t cnt)
{
	...
	if(ltype >=0 &&  ltype<L_MAX)
	{
		if(ltype == L_BATTERY){
			...
			if(l_type[L_BATTERY] == val)
				goto errout;
		}	
		l_type_change_mode(ltype,val);
	}
		...
		enum AW2023_LED_MODE mode)
{
	...
	switch(mode) {
		...
		case AW210XX_LED_POWERON_MODE:
			...
			effect_state.state[AW210XX_LED_GAMEMODE] = 0;
			effect_state.state[AW210XX_LED_POWERON_MODE] = 1;
			break;
		case AW210XX_LED_GAMEMODE:
			led->pdata->led_mode = AW210XX_LED_GAMEMODE;
			effect_state.state[AW210XX_LED_GAMEMODE] = 1;
			effect_state.state[AW210XX_LED_POWERON_MODE] = 0;
			break;
		default:
...
int check_priority_effect_pause(enum lights_types ltype){
	int i=0;
	for( i = L_MAX-1; i>ltype; i--)	{
		if( l_type[i] == 1) {
			return 0;
		}
	}
...
static int l_type_change_mode(enum lights_types ltype, int val) {
	...
	switch(ltype) {
		case L_BATTERY:
			if(val == 0){
				...
				l_type[L_BATTERY] = 1;
				effect_state.state[AW210XX_LED_CHARGE_MODE] = 1;
			}
			break;
		case L_NOTIFY:
			if(val == 0){
				l_type[L_NOTIFY] = 0;
				effect_state.state[AW210XX_LED_NOTIFY_MODE] = 0;
			}
			else {
				l_type[L_NOTIFY] = 1;
				effect_state.state[AW210XX_LED_NOTIFY_MODE] = 1;
		...
		case L_KEYBOARD:
			if(val == 0){
				l_type[L_KEYBOARD] = 0;
				effect_state.state[AW210XX_LED_MUSICMODE] = 0;
			}
			else {
				l_type[L_KEYBOARD] = 1;
				effect_state.state[AW210XX_LED_MUSICMODE] = 1;
				effect_state.data[AW210XX_LED_MUSICMODE] = 1;
			}
			break;
		case L_GAMES:
			if(val == 0){
				l_type[L_GAMES] = 0;
				effect_state.state[AW210XX_LED_GAMEMODE] = 0; //AW210XX_LED_POWERON_MODE
				effect_state.state[AW210XX_LED_POWERON_MODE] = 0;
			}
			else {
				l_type[L_GAMES] = 1;
		...
		case L_INCALL:
			if(val == 0){
				l_type[L_INCALL] = 0;
				effect_state.state[AW210XX_LED_INCALL_MODE] = 0;
			}
			else {
				l_type[L_INCALL] = 1;
				effect_state.state[AW210XX_LED_INCALL_MODE] = 1;
		...
		case L_WIFI:
			if(val == 0){
				l_type[L_WIFI] = 0;
				effect_state.state[AW210XX_LED_NEW_ALWAYSON] = 0;
			}
			else {
				l_type[L_WIFI] = 1;
				effect_state.state[AW210XX_LED_NEW_ALWAYSON] = 1;
				effect_state.data[AW210XX_LED_NEW_ALWAYSON] = 1;
			}
			break;
		default:
	...
	{ AW_LOG(" effect = %d , value = %d \n", i,effect_state.state[i]);}

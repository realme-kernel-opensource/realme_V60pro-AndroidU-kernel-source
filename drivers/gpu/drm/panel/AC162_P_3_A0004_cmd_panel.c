...
#include "AC162_P_3_A0004_cmd_panel.h"

#include "../mediatek/mediatek_v2/mtk_corner_pattern/ac162_p_3_a0004_data_hw_roundedpattern_l_boe.h"
#include "../mediatek/mediatek_v2/mtk_corner_pattern/ac162_p_3_a0004_data_hw_roundedpattern_r_boe.h"
...
static int lcm_panel_1p2_ldo_regulator_init(struct device *dev)
{
        ...
        if (IS_ERR_OR_NULL(vrf12_ldo)) { /* handle return value */
                ...
                DISP_ERR("get vrf12_ldo fail, error: %d\n", ret);
        }
...
static int lcm_panel_1p2_ldo_enable(struct device *dev)
{
	...
	if (!IS_ERR_OR_NULL(vrf12_ldo)) {
		...
		if (ret < 0)
			DISP_ERR("set voltage vrf12_ldo fail, ret = %d\n", ret);
		retval |= ret;
	}
        /* enable regulator */
	if (!IS_ERR_OR_NULL(vrf12_ldo)) {
		...
		if (ret < 0)
			DISP_ERR("enable regulator vrf12_ldo fail, ret = %d\n", ret);
		retval |= ret;
	}
...
static int lcm_panel_1p2_ldo_disable(struct device *dev)
{
	...
	if (!IS_ERR_OR_NULL(vrf12_ldo)) {
		...
		if (ret < 0)
			DISP_ERR("disable regulator vrf12_ldo fail, ret = %d\n", ret);
		retval |= ret;
	}
	return ret;
}
...
static int lcm_panel_1p8_ldo_regulator_init(struct device *dev)
{
        ...
        if (IS_ERR_OR_NULL(vrf18_ldo)) { /* handle return value */
                ...
                DISP_ERR("get vrf18_ldo fail, error: %d\n", ret);
        }
        regulator_1p8_inited = 1;
		DISP_INFO("get lcm_panel_1p8_ldo_regulator_init\n");
        return ret; /* must be 0 */
...
static int lcm_panel_1p8_ldo_enable(struct device *dev)
{
	...
	if (!IS_ERR_OR_NULL(vrf18_ldo)) {
		...
		if (ret < 0)
			DISP_ERR("set voltage vrf18_ldo fail, ret = %d\n", ret);
		retval |= ret;
	}
        /* enable regulator */
	if (!IS_ERR_OR_NULL(vrf18_ldo)) {
		...
		if (ret < 0)
			DISP_ERR("enable regulator vrf18_ldo fail, ret = %d\n", ret);
		retval |= ret;
	}
	DISP_INFO("get lcm_panel_1p8_ldo_enable\n");
...
static int lcm_panel_1p8_ldo_disable(struct device *dev)
{
	...
	if (!IS_ERR_OR_NULL(vrf18_ldo)) {
		...
		if (ret < 0)
			DISP_ERR("disable regulator vrf18_ldo fail, ret = %d\n", ret);
		retval |= ret;
	}
	return ret;
}
...
static void oplus_page_change_before_esd_read(struct drm_panel *panel, void *dsi,
                dcs_write_gce cb, void *handle)
{
	int i = 0;

	for (i = 0; i < sizeof(esd_check_multipage_pre)/sizeof(struct LCM_setting_table); i++) {
		cb(dsi, handle, esd_check_multipage_pre[i].para_list, esd_check_multipage_pre[i].count);
	}
	DISP_INFO("success.\n");
}
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.page_change_before_esd_read = oplus_page_change_before_esd_read,
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	...
	DISP_DEBUG("[LCM] %s+ ac162_p_3 a0004 Start\n", __func__);
	...
	register_device_proc("lcd", "AC162_A0004", "P_3");
...
/* #endif */ /* OPLUS_FEATURE_ONSCREENFINGERPRINT */
	...
	DISP_INFO("AC162_A0004_P_3 oplus_lcm_id2:%s lcm_id2:0x%x End\n", oplus_lcm_id2, lcm_id2);
	return ret;
}
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "ac162_p_3_a0004_cmd_panel",
		.owner = THIS_MODULE,
...
MODULE_AUTHOR("oplus");
MODULE_DESCRIPTION("ac162,p_3,a0004,OLED Driver");

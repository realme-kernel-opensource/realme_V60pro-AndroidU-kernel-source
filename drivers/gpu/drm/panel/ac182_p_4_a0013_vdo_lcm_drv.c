...
** File: ac182_p_4_a0013_vdo_lcm_drv.c
...
#include "../mediatek/mediatek_v2/mtk_corner_pattern/ac182_p_4_a0013_data_hw_roundedpattern.h"
#endif
#include "include/ac182_p_4_a0013_vdo_lcm_drv.h"
...
{
	char bl_tb1[] = {0xFF, 0x03, 0x98, 0x83, 0x08};
	char bl_tb2[] = {0xE0, 0x29, 0x55, 0x66, 0x69, 0x71, 0x7C, 0x55, 0x8D, 0xA0, 0xB1, 0xC9, \
					0xA9, 0xDE, 0x04, 0x28, 0x4C, 0xAA, 0x70, 0x99, 0xCA, 0xE9, 0xFF, 0x10, 0x2F, 0x58, 0x88, \
					0x3A, 0xAF, 0xC7, 0xDB};
	char bl_tb3[] = {0xE1, 0x29, 0x55, 0x66, 0x69, 0x71, 0x7C, 0x55, 0x8D, 0xA0, 0xB1, 0xC9, \
					0xA9, 0xDE, 0x04, 0x28, 0x4C, 0xAA, 0x70, 0x99, 0xCA, 0xE9, 0xFF, 0x10, 0x2F, 0x58, 0x88, \
					0x3A, 0xAF, 0xC7, 0xDB};
	char bl_tb4[] = {0xFF, 0x03, 0x98, 0x83, 0x00};
...
{
	char bl_tb1[] = {0xFF, 0x03, 0x98, 0x83, 0x08};
	char bl_tb2[] = {0xE0, 0x29, 0x00, 0x00, 0x18, 0x50, 0x85, 0x50, 0xC7, 0xFF, 0x2A, 0x5D, \
					0x95, 0x85, 0xC5, 0xF8, 0x25, 0xAA, 0x50, 0x7D, 0xB2, 0xD2, 0xFE, 0xFB, 0x1D, 0x48, 0x7C, \
					0x3A, 0xAF, 0xC7, 0xDB};
	char bl_tb3[] = {0xE1, 0x29, 0x00, 0x00, 0x18, 0x50, 0x85, 0x50, 0xC7, 0xFF, 0x2A, 0x5D, \
					0x95, 0x85, 0xC5, 0xF8, 0x25, 0xAA, 0x50, 0x7D, 0xB2, 0xD2, 0xFE, 0xFB, 0x1D, 0x48, 0x7C, \
					0x3A, 0xAF, 0xC7, 0xDB};
	char bl_tb4[] = {0xFF, 0x03, 0x98, 0x83, 0x00};
	...
	void *handle, unsigned int level)
{
	...
	if (level < 14 && level > 0 && g_gammaflag == 0) {
		g_gammaflag = 1;
		pr_debug("%s: backlight < 14 enter gamma!\n", __func__);
		lcm_gamma_enter(dsi, cb, handle);
	} else if (level > 13 && g_gammaflag == 1) {
		g_gammaflag = 0;
		pr_debug("%s: backlight > 13 exit gamma!\n", __func__);
	...
	pr_debug("%s: backlight =%d,bl0=0x%x,bl1=0x%x\n", __func__, level, bl_tb0[1], bl_tb0[2]);
	cb(dsi, handle, bl_tb2, ARRAY_SIZE(bl_tb2));
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	pr_err("[TP] ac182_p_4_a0013_vdo_lcm_drv g_shutdown_flag is %d, esd_flag is %d\n", g_shutdown_flag, esd_flag);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	register_device_proc("lcd", "A0013", "P_4");
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "ac182_p_4_a0013_vdo_lcm_drv",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("AC182 p 4 a0013 Panel Driver");

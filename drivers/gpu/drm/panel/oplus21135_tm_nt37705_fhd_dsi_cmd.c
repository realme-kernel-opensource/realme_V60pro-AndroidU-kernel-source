...
static const struct drm_display_mode display_mode[MODE_NUM * RES_NUM] = {
	...
	//vir_fhd_sdc_120_mode
	{
		...
		.hsync_start = FHD_FRAME_WIDTH + 9,//HFP
		.hsync_end = FHD_FRAME_WIDTH + 9 + 2,//HSA
		.htotal = FHD_FRAME_WIDTH + 9 + 2 + 21,//HBP
		.vdisplay = FHD_FRAME_HEIGHT,
		.vsync_start = FHD_FRAME_HEIGHT + 52,//VFP
		.vsync_end = FHD_FRAME_HEIGHT + 52 + 14,//VSA
		.vtotal = FHD_FRAME_HEIGHT + 52+ 14 + 22,//VBP
		.hskew = SDC_ADFR,
	},
	//vir_fhd_sdc_60_mode
	{
...
static struct mtk_panel_params ext_params[MODE_NUM] = {
	...
	.dyn_fps = {
		...
		.apollo_limit_superior_us = 10000, .apollo_limit_inferior_us = 13720,
	...
	.dyn_fps = {
		...
		.apollo_limit_superior_us = 2540, .apollo_limit_inferior_us = 10000,
	...
	unsigned int mode)
{
	...
	pr_debug("debug for nt37705 lcm %s  mode_mapping: %d->%d \n",
		__func__, mode, MODE_MAPPING_RULE(mode));
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	rc = of_property_read_u32(dev->of_node, "oplus-hpwm-bl-config", &pwm_bl_config);
	if (rc == 0) {
		hpwm_bl = pwm_bl_config;
		pr_info("lcm_probe hpwm_bl=0x%x\n", hpwm_bl);
	} else {
		hpwm_bl = 0;
		pr_info("lcm_probe hpwm_bl=%d\n", hpwm_bl);
	}

	rc = of_property_read_u32(dev->of_node, "oplus-hpwm-mode", &pwm_support);
	if (rc == 0) {
		hpwm_mode = pwm_support;
		pr_info("lcm_probe hpwm_mode=0x%x\n", hpwm_mode);
	} else {
		hpwm_mode = 0;
		pr_info("lcm_probe hpwm_mode=%d\n", hpwm_mode);
	}
